...
class TaskMonitor
{
    public:
        ...
        enum MsgId
        {
            ...
            REGISTER,           ///< Register task with the task monitor
        ...
        struct Message
        {
            Message(MsgId id = INVALID, osThreadId_t tId = nullptr, uint32_t maxTime = 0, void (*callback)() = nullptr)
                : msgId{id}, taskId{tId}, maxResponseTime{maxTime}, checkinCall{callback} {}
    ...
    private:
        ...
        /// @param maxResponseTime - Maximum time the task monitor will wait for a response from a task, from a checkin request
        /// @param checkinCall - function pointer to the checkin call to use when task monitor sends a message to the task
        void HandleRegister(osThreadId_t id, uint32_t maxResponseTime, void (*checkinCall)());
